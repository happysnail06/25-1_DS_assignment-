...
def merge_vocab(pair, vocab):
    ...
    for token_seq, freq in vocab.items():
        ...
        i = 0
        while i < len(tokens):
            # 만약 현재 토큰과 다음 토큰이 병합할 pair와 일치하면 두 토큰을 하나로 합칩니다.
            if i < len(tokens) - 1 and tokens[i] == pair[0] and tokens[i+1] == pair[1]:
                new_tokens.append(tokens[i] + tokens[i+1])
                i += 2  # 병합 후, 다음 두 토큰을 건너뜁니다.
            else:
                new_tokens.append(tokens[i])
                i += 1
        # 새로 병합된 토큰 시퀀스를 문자열로 변환합니다.
...
def compute_likelihood_score(pair, pair_freq, unigram_counts, total_tokens):
    ...
    expected_freq = (unigram_counts[pair[0]] * unigram_counts[pair[1]]) / total_tokens
    if expected_freq == 0:
        return 0
